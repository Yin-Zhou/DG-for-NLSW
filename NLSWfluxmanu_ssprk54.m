function [xloc,utrue,ucloc,err_ureal,energy,time] = NLSWfluxmanu_ssprk54(q,N,T,alpha,tau,beta)

% Solve u_tt - u_xx - 2iu_t - u = 0 in 1D 0 < x < 2pi by DG
...
u = [(sin(t)*cos(xloc+t))'; (sin(t)*sin(xloc+t))'];
grad_u = [(-sin(t)*sin(xloc+t))' ; (sin(t)*cos(xloc+t))'];
% ut = exp(i(x+2t))
v = [(cos(t)*cos(xloc+t) - sin(t)*sin(xloc+t))'; (cos(t)*sin(xloc+t) + sin(t)*cos(xloc+t))'];
...
for j = 1:N
            ...
            vt_real(:,j) = -2*Mv*(v_imag(:,j))-Su*(u_real(:,j));
            ...
            vt_imag(:,j) = 2*Mv*(v_real(:,j))-Su*(u_imag(:,j));
