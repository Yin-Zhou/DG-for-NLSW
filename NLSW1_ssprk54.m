function [xloc,utrue_real,ucloc_real,err_ureal,energy,time] = NLSW1_ssprk54(q,N,T,alpha,tau,beta)
...
for j=1:N
    ...
    energy(1,1) = energy(1,1) + (h(j)/2)*(((2/h(j))*S*u_real(:,j) - sin(xloc(:,j))).*W)'*((2/h(j))*S*u_real(:,j) - sin(xloc(:,j)));
    energy(1,1) = energy(1,1) + (h(j)/2)*(((2/h(j))*S*u_imag(:,j) + cos(xloc(:,j))).*W)'*((2/h(j))*S*u_imag(:,j) + cos(xloc(:,j)));
    
    energy(1,1) = energy(1,1) + (h(j)/2)*((P*u_real(:,j) + cos(xloc(:,j))).*W)'*(P*u_real(:,j) + cos(xloc(:,j)));
    energy(1,1) = energy(1,1) + (h(j)/2)*((P*u_imag(:,j) + sin(xloc(:,j))).*W)'*(P*u_imag(:,j) + sin(xloc(:,j)));
end

for it = 1:nsteps
    
    [rhsu_real, rhsv_real, rhsu_imag, rhsv_imag] = wave_schrodinger_update(q, N, h, S, xloc, W, P, alpha, Vb, v_real, Uxb, u_real, tau, beta, v_imag, u_imag);
    ...
    [rhsu_real, rhsv_real, rhsu_imag, rhsv_imag] = wave_schrodinger_update(q, N, h, S, xloc, W, P, alpha, Vb, v_real1, Uxb, u_real1, tau, beta, v_imag1, u_imag1);
    ...
    [rhsu_real, rhsv_real, rhsu_imag, rhsv_imag] = wave_schrodinger_update(q, N, h, S, xloc, W, P, alpha, Vb, v_real2, Uxb, u_real2, tau, beta, v_imag2, u_imag2);
    ...
    [rhsu_real3, rhsv_real3, rhsu_imag3, rhsv_imag3] = wave_schrodinger_update(q, N, h, S, xloc, W, P, alpha, Vb, v_real3, Uxb, u_real3, tau, beta, v_imag3, u_imag3);
    ...
    [rhsu_real, rhsv_real, rhsu_imag, rhsv_imag] = wave_schrodinger_update(q, N, h, S, xloc, W, P, alpha, Vb, v_real4, Uxb, u_real4, tau, beta, v_imag4, u_imag4);
    ...
    for j=1:N
        ...
        energy(it+1,1) = energy(it+1,1) + (h(j)/2)*(((2/h(j))*S*u_real(:,j) - sin(xloc(:,j))).*W)'*((2/h(j))*S*u_real(:,j) - sin(xloc(:,j)));
        energy(it+1,1) = energy(it+1,1) + (h(j)/2)*(((2/h(j))*S*u_imag(:,j) + cos(xloc(:,j))).*W)'*((2/h(j))*S*u_imag(:,j) + cos(xloc(:,j)));
        
        energy(it+1,1) = energy(it+1,1) + (h(j)/2)*((P*u_real(:,j) + cos(xloc(:,j))).*W)'*(P*u_real(:,j) + cos(xloc(:,j)));
        energy(it+1,1) = energy(it+1,1) + (h(j)/2)*((P*u_imag(:,j) + sin(xloc(:,j))).*W)'*(P*u_imag(:,j) + sin(xloc(:,j)));
    end
...
for j=1:N
    [uloc, grad_uloc, vloc] = soltrue(xloc(:,j),T);
    utrue_real(:,j) = (uloc(1,:)');
    grad_utrue_real(:,j) = (grad_uloc(1,:)');
    vtrue_real(:,j) = (vloc(1,:)');
  
%   test for real part of u, L2 or H1 norm
    ucloc_real(:,j) = (P*u_real(:,j)) + cos(xloc(:,j));
    grad_ucloc_real(:,j) = (2/h(j))*S*u_real(:,j) + sin(xloc(:,j));
...
%         *((grad_ucloc_real(:,j) - grad_utrue_real(:,j)).*W)'*(grad_ucloc_real(:,j) - grad_utrue_real(:,j));

end

% plot(xloc(:),ucloc_real(:),'b--',xloc(:),utrue_real(:),'r--')
% plot(xloc(:),utrue_real(:)-ucloc_real(:),'b--')
...
function [ut_real, vt_real, ut_imag, vt_imag] = wave_schrodinger_update(q, N, h, S, xloc, W, P, alpha, Vb, v_real, Uxb, u_real, tau, beta, v_imag, u_imag)
...
for j = 1:N
            ...
            if (j==1)
                ...
                jump = (2/h(N))*(Uxb(2,:)*(u_real(:,N))) - (2/h(j))*(Uxb(1,:)*(u_real(:,j)));
                ...
                wstarl_real = (1-alpha)*((2/h(N))*Uxb(2,:)*(u_real(:,N)));
                wstarl_real = wstarl_real + alpha*((2/h(j))*Uxb(1,:)*(u_real(:,j)));
                ...
                jump = (2/h(N))*Uxb(2,:)*(u_imag(:,N)) - (2/h(j))*(Uxb(1,:)*(u_imag(:,j)));
                ...
                wstarl_imag = (1-alpha)*((2/h(N))*Uxb(2,:)*(u_imag(:,N)));
                wstarl_imag = wstarl_imag + alpha*((2/h(j))*Uxb(1,:)*(u_imag(:,j)));
            ...
            elseif (j==N)
                ...
                jump = ((2/h(j))*Uxb(2,:)*(u_real(:,j))) - ((2/h(1))*Uxb(1,:)*(u_real(:,1)));
                vstarr_real = vstarr_real - tau*jump;
                ...
                wstarr_real = (1-alpha)*((2/h(j))*Uxb(2,:)*(u_real(:,j)));
                wstarr_real = wstarr_real + alpha*((2/h(1))*Uxb(1,:)*(u_real(:,1)));
                ...
                jump = ((2/h(j))*Uxb(2,:)*(u_imag(:,j))) - ((2/h(1))*Uxb(1,:)*(u_imag(:,1)));
                vstarr_imag = vstarr_imag - tau*jump;
                ...
                wstarr_imag = (1-alpha)*((2/h(j))*Uxb(2,:)*(u_imag(:,j)));
                wstarr_imag = wstarr_imag + alpha*((2/h(1))*Uxb(1,:)*(u_imag(:,1)));
            ...
            vt_real(:,j) = vt_real(:,j) - (h(j)/2)*(P')*(diag(W))*(2*cos(xloc(:,j)));
            ...
            vt_imag(:,j) = vt_imag(:,j) - (h(j)/2)*(P')*(diag(W))*(2*sin(xloc(:,j)));
