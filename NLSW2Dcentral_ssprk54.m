function [xloc,yloc,err_ureal] = NLSW2Dcentral_ssprk54(qx,qy,Nx,Ny,T)
...
for it = 1:nsteps
    ...
    [rhsu_real3, rhsv_real3, rhsu_imag3, rhsv_imag3] = wave_schrodinger_update(qx, qy, hx, hy, Nx, Ny, Mu, Su, Vb_x, Vb_y, v_real3, Uxb_x, Uxb_y, u_real3, ...
        v_imag3, u_imag3);
    u_real4 = c40*u_real + c43*u_real3 + c44*dt*rhsu_real;
    v_real4 = c40*v_real + c43*v_real3 + c44*dt*rhsv_real;   
    u_imag4 = c40*u_imag + c43*u_imag3 + c44*dt*rhsu_imag;
    v_imag4 = c40*v_imag + c43*v_imag3 + c44*dt*rhsv_imag;
...
for i=1:Nx
    for j=1:Ny
        [uloc, vloc] = solnew(xloc(:,i),yloc(:,j),T);
        
%         uexact(((j-1)*(qy+1)+1):(j*(qy+1)), ((i-1)*(qx+1)+1):(i*(qx+1))) = reshape((uloc(1,:)'),(qy+1),(qx+1));
