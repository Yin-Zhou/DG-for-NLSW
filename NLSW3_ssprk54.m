function [xloc,utrue,ucloc,err_ureal,energy,time] = NLSW3_ssprk54(q,N,T,alpha,tau,beta)

% Solve u_tt - u_xx + iu_t + e^{-x^2} u = 0 in 1D 0 < x < 2pi by DG
...
for j = 1:N
            ...
            Mu1 = (2/h(j))*(S')*diag(W)*S + (h(j)/2)*(P')*(diag(W)*diag(exp(-((xloc(:,j)).^2))))*P;
            line(1,1) = h(j)/2;
            Mu = [line;Mu1];
%             if j == 1
%               disp(Mu);
...
%             disp(det(Mu));
            ...
            Fu_real = [0;(Fu_real')]-Sv*(v_real(:,j));
            ut_real(:,j) = Mu\Fu_real;
...
%             ut_real(:,j) = Mu\(ut_real(:,j));
            ...
            Fu_imag = [0;(Fu_imag')]-Sv*(v_imag(:,j));
            ut_imag(:,j) = Mu\Fu_imag;
